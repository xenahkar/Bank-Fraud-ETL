...
# подключение к бд
connection = psycopg2.connect(**Config.remote)
    ...
    try:
        ...
        for table in stg_tables:
            try:
                ...
                print(f"Ошибка в очищении таблицы {table}: {te}")
        connection.commit()
...
transactions = pd.read_csv(os.path.join(data_folder, transactions_files[0]), sep=';', encoding='utf-8')
